// pipes_win.cpp — pipes.sh-like (Windows 11): pre-run menu + deferred color + responsive
...
struct Term {
  ...
  void init(){
    ...
    DWORD m=0; hout=GetStdHandle(STD_OUTPUT_HANDLE); GetConsoleMode(hout,&m);
    m |= ENABLE_VIRTUAL_TERMINAL_PROCESSING; SetConsoleMode(hout,m);
    hin = GetStdHandle(STD_INPUT_HANDLE);
    updateSize(); hideCursor();
  }
  void restore(){ showCursor(); }
  void updateSize(){
  ...
  bool checkResize(){ int ow=W, oh=H; updateSize(); if (ow!=W || oh!=H){ clear(); return true; } return false; }
